...
class ContactsViewModel(val mApplication: Application) : BaseViewModel(mApplication) {
    ...
    fun fetchContacts() {
        viewModelScope.launch {
            ...
            val contacts: ArrayList<Contact> = contactsListAsync.await()
            val contactNumbers: HashMap<String, ArrayList<String>> = contactNumbersAsync.await()
            val contactEmails = contactEmailAsync.await()

            contacts.forEach {
                contactNumbers[it.id]?.let { numbers ->
                    if (numbers.size>0) {
                        val arrayList = numbers.get(0)
                        it.numbers.add(arrayList)
                    } else {
