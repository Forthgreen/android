...
class ProfileDetailsInteractor {
    ...
    companion object {
        private const val TAG = "ProfileDetailsInteractor"
    }

    fun fetchUserDetails(body: RequestBody, networkRequestCallbacks: NetworkRequestCallbacks): Disposable {
        return RestClient.get().fetchUserDetails(ApplicationGlobal.accessToken, body)
