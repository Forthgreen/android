...
class SearchUsersInteractor {
    ...
    companion object {
        private const val TAG = "SearchUsersInteractor"
    }

    fun searchUsers(body: RequestBody, networkRequestCallbacks: NetworkRequestCallbacks): Disposable {
        return RestClient.get().searchUsers(ApplicationGlobal.accessToken, body)
